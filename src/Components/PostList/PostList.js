...
const PostList = () => {
          ...
          <li style={{ listStyleType: "none" }} key={post.id}>
            {editingId === post.id ? (
              <form onSubmit={handleUpdate}>
                ...
                <button className="btn btn-warning mx-3" type="button" onClick={handleCancel}>
                  Cancel
                </button>
                </div>
              </form>
  ...
  <tbody>
    <tr>
      ...
      <td><button className="btn btn-warning" type="button" onClick={() => handleEdit(post)}>Edit</button></td>
      <td><button className="btn btn-danger mx-3" type="button" onClick={() => dispatch(deletePostAsync(post.id))}>Delete</button></td>
    </tr>
  </tbody>
</table>
